...
class ExperimentBuilder(nn.Module):
    def __init__(self, network_model, experiment_name, num_epochs, train_data, val_data,
                 test_data, weight_decay_coefficient, use_gpu, l_rate, continue_from_epoch=-1):
        """
        ...
        self.optimizer = optim.Adam(self.parameters(), amsgrad=False,
                                    weight_decay=weight_decay_coefficient, lr=l_rate)
    ...
    def plot_grad_flow(self, named_parameters):
        ...
        """
        for names, params in named_parameters:
            if 'bias' not in names:
                names = str(names)
                names = names.split('.')
                names = names[1] + '_' + names[-2]
                layers.append(names)
